...
class LogisticRegression(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        logits = torch.mm(x, self.weights) + self.bias
...
def main():
    ...
    plot(epochs, losses, name=f'{opt.output}-training-loss', ylim=ylim)
    #accuracy = { "Valid Accuracy": valid_accs }
    #plot(epochs, accuracy, name=f'{opt.output}-validation-accuracy', ylim=(0., 1.))
